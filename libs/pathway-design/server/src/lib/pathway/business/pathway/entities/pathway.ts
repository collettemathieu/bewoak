...
import { PathwayInitializedEvent } from '../events/pathway-initialized.event';
...
import { INVALID_PATHWAY_DATA_INITIALIZATION_MESSAGE } from './pathway.constants';
import type { AddArticleParams, InitializePathwayParams } from './pathway.types';

export class PathwayEntity extends AggregateRoot {
    ...
    initialize({ description, pathwayId, researchField, title }: InitializePathwayParams) {
        ...
        if (haveErrors(failures)) {
            return failure(
                new CTSEBadRequestException(
                    INVALID_PATHWAY_DATA_INITIALIZATION_MESSAGE,
        ...
        this.apply(
            new PathwayInitializedEvent(this.pathwayId, {
